...
model HeatingIdeal
"Ideal heating energy transfer station"
  ...
  Buildings.Experimental.DHC.EnergyTransferStations.Heating.Generation1.Subsystems.BaseClasses.SteamTrap steTra(
    show_T=show_T,             redeclare package Medium = Medium_b,
      m_flow_nominal=m_flow_nominal) "Steam trap"
    annotation (Placement(transformation(extent={{-20,-10},{0,10}})));
equation
  connect(pum.P, PPum) annotation (Line(points={{61,9},{70,9},{70,60},{110,60}},
        color={0,0,127}));
  connect(port_a, hex.port_a)
    annotation (Line(points={{-100,0},{-80,0}}, color={0,127,255}));
  connect(mAct_flow.y, pum.m_flow_in)
    annotation (Line(points={{21,54},{50,54},{50,12}},color={0,0,127}));
  ...
  connect(hex.dh, inv.u) annotation (Line(points={{-59,6},{-50,6},{-50,30},{-42,
          30}}, color={0,0,127}));
  connect(hex.port_b, steTra.port_a)
